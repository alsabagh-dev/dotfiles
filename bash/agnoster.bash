...
prompt_dir() {
    ...
    location=`pwd`  
    DIRNAME=${location/#'/home/abdelaziz'/'🏠 '}
    if [[ "$DIRNAME" =~ ([^/]+/+[^/]+)/*$ ]]; then
        prompt_segment blue black "${BASH_REMATCH[1]}"
    else
        prompt_segment blue black $DIRNAME
    fi
}
...
set_bash_prompt() {
    ...
    if test -t 1; then
        ...
        if test -n "$ncolors" && test $ncolors -ge 9; then
            ...
            PR='\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]\w\[\033[00m\]\$ '
        fi
    else
        PR='\u@\h:\w\$ '
    fi
